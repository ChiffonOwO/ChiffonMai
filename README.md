# ChiffonMai

<div align="center">
  <img src="https://img.shields.io/badge/Platform-Android/iOS-blue.svg" alt="Platform">
  <img src="https://img.shields.io/badge/Language-Dart/Flutter-blue.svg" alt="Language">
  <img src="https://img.shields.io/badge/Purpose-Maimai%20DX%20Tool-green.svg" alt="Purpose">
</div>

<br>

ChiffonMai 是一款专为 **舞萌DX 2025(maimai DX 2025)** 玩家打造的一站式移动端工具类应用，聚合了曲库查询、成绩统计、Rating计算、曲目推荐等多种实用功能，助力玩家提升游玩体验。

## 🎉 特别致谢

- 感谢 **Xn1xUfguF1GiNg** 为本APP提供的背景图和戚风小狐狸（真的非常可爱）
- 感谢 **Takanashi Rikkkkkkka** 为本APP的个性化谱面推荐功能提供的宝贵的设计思路
- 感谢 **三由yyyh** 为本APP的页面设计提供的宝贵意见
- 感谢 **MYD** 提供的用于测试开发的个人游玩数据
- 感谢 [Diving-Fish](https://www.diving-fish.com/maimaidx/prober/) 的接口提供的谱面数据、玩家数据等
- 感谢 [DXRating.net](https://dxrating.net) 提供的谱面标签数据库

## ✨ 核心功能

| 功能模块 | 功能描述 |
|---------|---------|
| 🎵 乐曲查询 | 完整查询舞萌DX曲库的所有乐曲信息 |
| 📊 成绩查询 | 查看个人游玩数据、成绩记录 |
| 🏆 Best50查询 | 计算并展示个人Best50成绩 |
| 📈 拟合Best50查询 | 分析拟合Best50拟合情况 |
| 🎯 基于Best50推荐 | 根据你的Best50成绩智能推荐适合的曲目 |
| 🎼 基于流派推荐 | 按你常玩的曲目流派个性化推荐 |
| 🏷️ 基于标签推荐 | 基于谱面标签（如难度、风格）精准推荐曲目 |
| 🔀 随机乐曲 | 随机生成1-4首曲目 |
| 🧮 单曲Rating计算 | 计算单曲Rating |
| 📉 达成率计算 | 根据判定详情（Perfect/Great等）精准算出达成率 |
| 🆚 版本对照 | 版本名对照版本号，如maimai GreeN对应超代 |
| 🔍 达成率反推 | 根据达成率反推绝赞判定详情 |
| 📱 绑定二维码 | 关联你的舞萌DX账号，同步游戏数据 |


## ✨ 部分功能展示
<div align="center">
  <img src="showImg/homePage.jpg" width="20%" alt="首页展示"/>
  <img src="showImg/reversePage.jpg" width="20%" alt="达成率反推展示"/>
  <img src="showImg/scoreCalculatorPage.jpg" width="20%" alt="Rating计算展示"/>
  <img src="showImg/scoreResult.jpg" width="20%" alt="Rating计算结果展示"/>
</div>


## 📱 适配说明

- 支持Android/iOS双平台

## 🛠️ 技术栈

- 开发框架：Flutter
- 编程语言：Dart
- 状态管理：
- 网络请求：
- 本地存储：

## 📥 安装方式

### 安卓端

1. 下载最新版APK文件（Releases页面）
2. 允许安装未知来源应用
3. 安装并打开应用

### iOS端

1. 通过TestFlight测试版安装 或
2. App Store上架后直接搜索"ChiffonMai"下载

## 📄 隐私说明

- 应用仅在本地存储你的游戏数据，不会上传至第三方服务器
- 账号绑定仅用于同步官方数据，不会收集敏感信息
- 所有计算逻辑均在本地完成，保障数据安全

## 📝 许可证

本项目采用 [MIT License](LICENSE) 开源许可证，详细条款请查看LICENSE文件。

## 💡 免责声明

- ChiffonMai 是第三方工具，非SEGA官方出品
- 本应用仅用于学习和娱乐，请勿用于商业用途
- 数据来源均为公开信息，如有侵权请联系删除

## 📞 反馈与建议

- 邮箱：488581724@qq.com

## 🎉 致谢

- 感谢所有舞萌DX玩家的测试和反馈
- 感谢maimaiDX相关数据开源社区的支持
- 致敬SEGA开发的优秀音游作品